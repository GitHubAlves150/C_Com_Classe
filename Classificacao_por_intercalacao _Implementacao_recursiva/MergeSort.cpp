...
{
    size = (vectorSize > 0 ? vectorSize :10);     //se vector for maior que 0, então seja vectorSize senão seja 10
                                                  //Valida vectorSize
...
void MergeSort::sortSubvector(int low, int high)           //exibe elementos do vetor
{
    ...
    if( (high - low) >=1 ) //se não for o caso básico
    {
        ...
        sortSubvector(low, media1);       //primeira metade do vetor
        sortSubvector(media2, high);      //segudna metade do vetor
...
void MergeSort::_merge(int left, int media1, int media2, int right )        //mescla os dois vetores classificados
{
    ...
    vector< int >combned(size);            // vetor de trabalho
